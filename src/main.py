...
def runOneStrat(strategy, data, data_df, timeframe, startdate, enddate, corrmethod, reb_period, shares_list, shareclass,
                lookback_period_short, lookback_period_long, moving_average_period, momentum_period, momentum_percentile):
    ...
    i = 0
    for dt in data:
        data_df[shares_list[i]] = dt.close
    ...
    if list(filter(None, params['weights'])) != [] and strategy == 'customweights':
        weights_list = list(filter(None, params['weights']))
...
def main(params):
    ...
    if params['initial_cash'] == '': return False

    strategy_list = []
    if params['riskparity']: strategy_list.append('riskparity')
    if params['riskparity_nested']: strategy_list.append('riskparity_nested')
    if params['rotationstrat']: strategy_list.append('rotationstrat')
    if params['uniform']: strategy_list.append('uniform')
    if params['vanillariskparity']: strategy_list.append('vanillariskparity')
    if params['onlystocks']: strategy_list.append('onlystocks')
    if params['sixtyforty']: strategy_list.append('sixtyforty')
    if params['trend_u']: strategy_list.append('trend_u')
    if params['absmom_u']: strategy_list.append('absmom_u')
    if params['relmom_u']: strategy_list.append('relmom_u')
    if params['momtrend_u']: strategy_list.append('momtrend_u')
    if params['trend_rp']: strategy_list.append('trend_rp')
    if params['absmom_rp']: strategy_list.append('absmom_rp')
    if params['relmom_rp']: strategy_list.append('relmom_rp')
    if params['momtrend_rp']: strategy_list.append('momtrend_rp')
    if params['GEM']: strategy_list.append('GEM')
    if params['acc_dualmom']: strategy_list.append('acc_dualmom')
    if params['acc_dualmom2']: strategy_list.append('acc_dualmom2')
    if params['rot_adm']: strategy_list.append('rot_adm')
    if params['rot_adm_dual_rp']: strategy_list.append('rot_adm_dual_rp')
    if params['vigilant']: strategy_list.append('vigilant')
    if params['TESTING']: strategy_list.append('TESTING')
    if params['specific_vigilant']: strategy_list.append('specific_vigilant')
    if params['specific_rot']: strategy_list.append('specific_rot')
    if params['specific_rot2']: strategy_list.append('specific_rot2')
    if params['specific_adm']: strategy_list.append('specific_adm')
    if params['specific_adm_grad_div']: strategy_list.append('specific_adm_grad_div')
    if params['specific_fabio_adm2']: strategy_list.append('specific_fabio_adm2')
    if params['specific_fabiofg_adm2']: strategy_list.append('specific_fabiofg_adm2')
    if params['specific_fabio_adm3']: strategy_list.append('specific_fabio_adm3')
    if params['specific']: strategy_list.append('specific')
    if not strategy_list: strategy_list = ["customweights"]
    if params['benchmark'] != '': strategy_list = strategy_list + ['benchmark']

    print_header(params,strategy_list)

    # Set the timeframe
    if params['historic'] == 'long': timeframe = bt.TimeFrame.Years
    else: timeframe = bt.TimeFrame.Days

    params_weights = list(filter(None, params['weights']))
    ...
    if params_weights != [] or strategy_list == ["customweights"]:
    ...
    # end loan if
    data = common_dates(data=data, shareclass=shareclass, fromdate=startdate, todate=enddate, timeframe=timeframe)
    data_df = pd.DataFrame(index=data[0].index)
    ...
    for strat in strategy_list:
        ...
        ThisOutputList = runOneStrat(strat, data, data_df, timeframe, startdate, enddate, corrmethod, reb_period, shares_list, shareclass,
                                     lookback_period_short, lookback_period_long, moving_average_period, momentum_period, momentum_percentile)
